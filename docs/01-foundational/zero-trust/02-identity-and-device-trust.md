...
description: Master modern identity attestation, SPIFFE/SPIRE workload identities,
  hardware-backed device posture verification, and multi-language context-aware authorization
  code patterns.
keywords:
- Identity
- Trust
...
- Contextual
- AuthZ
slug: /foundational/zero-trust/identity-and-device-trust
---
...
| :--- | :--- | :--- |
| **Clock Drift Across Services** | Valid JWT tokens rejected due to `nbf` (Not Before) or `exp` mismatch. | Enforce Network Time Protocol (NTP) synchronization across nodes; set JWT leeway tolerance (`30-60 seconds`). |
