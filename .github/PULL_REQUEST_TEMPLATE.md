## Description
<!-- What does this PR add or fix? -->

## Type of Change
- [ ] New guide (complete)
- [ ] Guide update / expansion
- [ ] Bug fix / error correction
- [ ] Website / tooling change
- [ ] Typo / formatting fix

## Which section / guide does this affect?
<!-- e.g., docs/04-ai-ml-security/llm-prompt-injection/ -->

## Checklist
- [ ] I have read [CONTRIBUTING.md](../CONTRIBUTING.md)
- [ ] My guide follows the [standard module structure](community/templates/guide-template.md)
- [ ] All code examples are tested and working
- [ ] I have cited sources in `07-references.md`
- [ ] No proprietary or copyrighted content included without permission
- [ ] I agree my contribution is licensed under CC BY 4.0

## Screenshots / Preview
<!-- If you made website or formatting changes, add a screenshot -->

## Related Issues
<!-- Closes #issue_number -->
